...
:::note
If the embedding model is not specified, the library will use the default one for that LLM, if available.
:::

## Local Models

Currently, local models are supported through Ollama integration. Ollama is a provider of LLM models which can be downloaded from [Ollama](https://ollama.com/).
...
You can get the API key from [here](https://ai.google.dev/gemini-api/docs/api-key).
...
You can get the API key from [here](https://console.groq.com/keys). Groq doesn't support embedding models, so in the following example we are using Ollama one.
...
- [chat model list](https://python.langchain.com/v0.2/docs/integrations/chat/#all-chat-models)
- [embedding model list](https://python.langchain.com/v0.2/docs/integrations/text_embedding/#all-embedding-models)
