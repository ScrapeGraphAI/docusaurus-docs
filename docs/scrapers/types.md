...
:::note
They all use a graph configuration to set up LLM models and other parameters. To find out more about the configurations, check the LLM and Configuration sections.
:::
...
OmniScraperGraph is designed to scrape both text and images from web pages, with the ability to describe image content.
...
OmniSearchGraph extends the search capabilities to include image content analysis.
...
SmartScraperGraph is the core scraping component that extracts structured information from web pages.
...
SearchGraph combines search engine results with smart scraping capabilities.
...
SpeechGraph adds text-to-speech capabilities to the scraping pipeline.
...
ScriptCreatorGraph generates Python scripts for custom web scraping tasks.
